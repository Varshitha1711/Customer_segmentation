...
def load_model():
    ...
    try:
        model = joblib.load(r"C:\Users\91934\OneDrive\Desktop\AI&DS-Tekworks\KMeans\Clustering\tree_model.pkl")
        return model
...
def load_data():
    ...
    try:
        df = pd.read_csv(r"C:\Users\91934\OneDrive\Desktop\AI&DS-Tekworks\KMeans\Clustering\Mall_Customers.csv")
        return df
    except FileNotFoundError:
...
if model is not None and df is not None:
    ...
    if st.button("🚀 Predict Cluster", use_container_width=True, key="predict_btn"):
        ...
        with col1_info:
            ...
            st.markdown(f"""
                <div class="cluster-info">
                    <p style="color: #333;" ><strong>{cluster_details['description']}</strong></p>
                </div>
            """, unsafe_allow_html=True)
        ...
        with col2_info:
            ...
            clustered_data = pd.read_csv(r"C:\Users\91934\OneDrive\Desktop\AI&DS-Tekworks\KMeans\Clustering\clustered_mall_Customers_.csv")
            cluster_stats = clustered_data[clustered_data['clusters'] == cluster]
            
            st.markdown(f"""
                <div class="cluster-info">
                    <p style="color: #333;"><strong>Size:</strong> {len(cluster_stats)} customers ({len(cluster_stats)/len(clustered_data)*100:.1f}%)</p>
                    <p style="color: #333;"><strong>Avg Age:</strong> {df[df.index.isin(cluster_stats.index)]['Age'].mean():.1f} years</p>
                    <p style="color: #333;"><strong>Avg Income:</strong> ${df[df.index.isin(cluster_stats.index)]['Annual Income (k$)'].mean():.1f}k</p>
                    <p style="color: #333;"><strong>Avg Spending:</strong> {df[df.index.isin(cluster_stats.index)]['Spending Score (1-100)'].mean():.1f}/100</p>
                </div>
            """, unsafe_allow_html=True)
        ...
        with viz_col1:
            ...
            clustered_df = pd.read_csv(r"C:\Users\91934\OneDrive\Desktop\AI&DS-Tekworks\KMeans\Clustering\clustered_mall_Customers_.csv")
            ...
            fig = px.scatter_3d(
                ...
                labels={'x': 'Age', 'y': 'Annual Income', 'z': 'Spending Score'},
